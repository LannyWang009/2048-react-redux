import { UP, DOWN, RIGHT, LEFT, RENEW } from '../actionTypes'
...
class Matrix {
  ...
  checkisSuccess = matrix => {
    ...
    const check = foo => {
      this.matrix = copy;
      const isMoved = this.isBoardMoved(copy, foo().matrix);
  ...
  addRandomNumToMatrix = () => {
    ...
    if (!this.isMoved) {
      if (this.checkisSuccess(newMatrix)) {
        this.isSuccess = true;
    ...
    if (this.checkisSuccess(newMatrix)) {
      this.isSuccess = true;
      return { isSuccess: true, matrix: newMatrix };
    }
